...
| `dws mail attachment download` | 下载邮件附件到本地 |
...
用户说"下载附件/保存附件/把附件存到本地" → 先 `message search` 获取 messageId，再 `attachment list` 获取 attachmentId 和 name，最后 `attachment download`
...
# 5. 下载邮件附件到本地
# 步骤 5.1：列出邮件附件，获取 attachmentId 和 name
...
dws mail attachment download --email user@company.com \
  --message-id <messageId> --attachment-id <attachmentId> --name report.pdf --output /tmp
