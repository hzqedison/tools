...
- [ ] 是否需要传 `--column-keywords`？（用户提到了"迟到/加班/工时/异常"等特定关注维度才传）
- [ ] 报错时如何向用户解释？
...
> **默认报表类型**：用户未指明报表类型时，**默认走月度汇总**，事后告知"已按月度汇总输出，如需明细/每日统计请告知"。

### 列选择（独立维度，与报表类型正交）

| 用户说 | 列选择方式 |
|--------|-----------|
| "帮我出一份考勤报表" / "导出考勤" / 未提及特定关注维度 | 使用报表类型对应的**预定义列集合** |
| "迟到情况报表" / "工作时长报表" / "异常报表" / "加班统计" / 提及了特定关注维度 | 通过 `--column-keywords` 传给脚本 |
...
| "帮我排班" | `dws attendance schedule export`（排班，非报表） |
...
1. **解析用户输入**（两个独立维度）：
   - **报表类型**（三选一）：明细 / 月度汇总（默认） / 每日统计
   - **列选择**（独立维度）：预定义列集合（默认） / 用户指定维度筛选
...
1. **用户未提及特定关注维度** → 不传 `--column-keywords`，脚本使用内置默认字段集
2. **用户提及了特定关注维度**（如"迟到情况"） → 传 `--column-keywords "迟到次数,迟到时长,..."`
3. **明细报表** → 列固定，不支持 `--column-keywords`
...
- `--end`（必填）：结束日期，同上
- `--column-keywords`（可选）：逗号分隔的字段名关键词。不传则使用脚本内置默认字段集
- `--out`（可选）：输出文件名，不传则自动生成

脚本内部自动处理：`report columns` 获取字段清单 → 按关键词匹配 → `report query-data` 分批分段查询 → `contact user get` 姓名映射 → 聚合计算 → 生成 Excel
...
| 时间超限（明细） | `--from` 到 `--to` 不超过 1 个月 | 自动按月分段 |
...
### 示例 2: 特定维度筛选
**用户说**: "帮我出一份研发组 4 月的迟到情况报表"

```bash
python scripts/attendance_report_monthly.py \
  --users userId1,userId2,... \
  --start "2026-04-01" --end "2026-04-30" \
  --column-keywords "迟到次数,迟到时长,严重迟到次数,严重迟到时长,旷工迟到次数"
```
