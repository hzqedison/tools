...
> ** 危险操作：文件将被移入回收站。执行前需要确认或传入 `--yes` 跳过确认。**

```
Usage:
  dws drive delete [flags]
Example:
  dws drive delete --file-id <dentryUuid> --yes    # 查询 fileId: dws drive list
  dws drive delete --file-id <dentryUuid>           # 交互式确认后删除
Flags:
